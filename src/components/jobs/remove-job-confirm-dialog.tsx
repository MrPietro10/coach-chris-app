...
export type RemoveJobConfirmOptions = {
  removeLinkedTailoredResumes: boolean;
};

type RemoveJobConfirmDialogProps = {
  open: boolean;
  jobTitle: string;
  onConfirm: (options: RemoveJobConfirmOptions) => void;
  onCancel: () => void;
};

export function RemoveJobConfirmDialog({
  open,
  jobTitle,
  onConfirm,
  onCancel,
}: RemoveJobConfirmDialogProps) {
  ...
  return (
    <div
      ...
      aria-describedby="remove-job-description"
    >
      <section className="w-full max-w-md rounded-xl border border-zinc-200 bg-white p-6 shadow-xl">
        <h2 id="remove-job-title" className="text-lg font-semibold text-zinc-900">
          Remove this job?
        </h2>
        <p id="remove-job-description" className="mt-2 text-sm text-zinc-600">
          <span className="font-medium text-zinc-800">{jobTitle}</span> will be removed from your
          workspace. Your resume versions stay saved unless you choose to remove linked tailored
          resumes below.
        </p>
        <label className="mt-4 flex items-start gap-2.5 text-sm text-zinc-700">
          ...
          />
          <span>Also remove tailored resumes created for this job</span>
        </label>
