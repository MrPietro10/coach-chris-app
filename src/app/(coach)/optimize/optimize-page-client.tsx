    ...
    <>
      ...
      <section className="rounded-xl border border-zinc-200/80 bg-white p-5">
        <p className="text-sm text-zinc-700">
          Guided optimization is not available in this alpha build yet.
        </p>
        <p className="mt-2 text-sm text-zinc-600">
          Open a saved job in Results, run analysis, and use the inline resume updates there.
        </p>
      </section>
    </>
  );
...
export default function OptimizePageClient() {
  const availableJobIds = Object.keys(optimizeByJob);
  if (availableJobIds.length === 0) {
    return <OptimizeEmptyState />;
  }

  return <OptimizeWorkspace availableJobIds={availableJobIds} />;
}

function OptimizeWorkspace({ availableJobIds }: { availableJobIds: string[] }) {
  const [selectedJobId, setSelectedJobId] = useState(availableJobIds[0]);
